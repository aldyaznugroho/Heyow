...
public class MoviesListAdapterBigTest extends RecyclerView.Adapter<MoviesListAdapterBigTest.MoviesHolder> {

    private Context context;
    private List<Movies> moviesDataList;
    private String matchDetailType;

    public MoviesListAdapterBigTest(Context context, List<Movies> moviesDataList, String matchDetailType) {
    ...
    public MoviesHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View moviesView = LayoutInflater.from(parent.getContext()).inflate(R.layout.match_view_holder_1, null);

        moviesView.setLayoutParams(new RecyclerView.LayoutParams(
                RecyclerView.LayoutParams.MATCH_PARENT,
                RecyclerView.LayoutParams.WRAP_CONTENT));

        return new MoviesHolder(moviesView);
    }
    ...
    class MoviesHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        private MoviesHolder(View itemView) {
            super(itemView);
            itemListLayout = (LinearLayout) itemView.findViewById(R.id.match_list_1_parent_layout);
            posterImage = (ImageView) itemView.findViewById(R.id.match_list_1_poster_image);
            movieTitle = (TextView) itemView.findViewById(R.id.match_list_1_title_text);
            movieYear = (TextView) itemView.findViewById(R.id.match_list_1_datetime_text);
